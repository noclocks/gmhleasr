...
#  ------------------------------------------------------------------------
#
# Title : Entrata Internal Functions
#    By : Jimmy Briggs
#  Date : 2024-09-05
#
#  ------------------------------------------------------------------------
...
req_retry_is_transient <- function(resp) {
  ...
  resp_body <- httr2::resp_body_json(resp)
  resp_status_code <- NULL
  resp_status_message <- NULL

  if (purrr::pluck_exists(resp_body, "response", "error", "code")) {
    resp_status_code <- purrr::pluck(resp_body, "response", "error", "code")
    # cli::cli_alert_info("Response Status Code: {resp_status_code}")
  }

  if (purrr::pluck_exists(resp_body, "response", "error", "message")) {
    resp_status_message <- purrr::pluck(resp_body, "response", "error", "message")
    # cli::cli_alert_info("Response Status Message: {resp_status_message}")
  }

  if (is.null(resp_status_code)) { return(FALSE) }
  if (resp_status_code == 310) { return(TRUE) }
